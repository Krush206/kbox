...
DateTime *DateTime_new_parse (const char *str, Error **error, const char *tz,
  ...
  if (DateTime_parse (str, "%e %b %Y %H:%M", &tm)) goto success;
  if (DateTime_parse (str, "%e %b %H:%M", &tm)) goto success;
  if (DateTime_parse (str, "%e %b %Y", &tm)) goto success;
  if (DateTime_parse (str, "%e %b", &tm)) goto success;
...
Caller must free string
=======================================================================*/
char *DateTime_time_to_string_UTC (const DateTime *self)
  {
  char *oldtz = NULL;
  char *tz = "GMT0";
  if (tz)
...
=======================================================================*/
int DateTime_get_day_of_year (const DateTime *self)
  {
  struct tm tm;
