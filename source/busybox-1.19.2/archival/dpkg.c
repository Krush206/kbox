...
static void write_status_file(deb_file_t **deb_file)
{
	...
	int write_flag;
	int i = 0;
    char status_file_name[MAX_FILENAME];
    char status_udeb_file_name[MAX_FILENAME];
    char status_udeb_bak_file_name[MAX_FILENAME];
    char new_status_file_name [MAX_FILENAME];
	FILE *old_status_file;
	FILE *new_status_file;

    snprintf (status_file_name, MAX_FILENAME - 1, 
      "%s/var/lib/dpkg/status", getenv("KBOX"));
    snprintf (new_status_file_name, MAX_FILENAME - 1,
      "%s/var/lib/dpkg/status.udeb", getenv("KBOX"));

	old_status_file = xfopen_for_read(status_file_name);
	new_status_file = xfopen_for_write(new_status_file_name);
    ...
    //char status_file_name[MAX_FILENAME];
    snprintf (status_file_name, MAX_FILENAME - 1, "%s/var/lib/dpkg/status",
      getenv("KBOX"));
    snprintf (status_udeb_file_name, MAX_FILENAME - 1, 
      "%s/var/lib/dpkg/status.udeb", getenv("KBOX"));
    snprintf (status_udeb_bak_file_name, MAX_FILENAME - 1, 
      "%s/var/lib/dpkg/status.udeb.bak", getenv("KBOX"));
	if (rename(status_file_name,status_udeb_bak_file_name) == -1) {
		if (errno != ENOENT)
	...
	xrename(status_udeb_file_name, status_file_name);
}
...
static void run_package_script_or_die(const char *package_name, const char *script_type)
{
	...
	script_path = xasprintf("%s/var/lib/dpkg/info/%s.%s", getenv("KBOX"), 
      package_name, script_type);
...
static char **all_control_list(const char *package_name)
{
	...
	while (i < ARRAY_SIZE(all_control_files)) {
		remove_files[i] = xasprintf("%s/var/lib/dpkg/info/%s.%s", 
           getenv("KBOX"), package_name, all_control_files[i]);
		i++;
	}
...
static void remove_package(const unsigned package_num, int noisy)
{
	...
	sprintf(list_name, "%s/var/lib/dpkg/info/%s.%s", getenv("KBOX"), 
       package_name, "list");
	remove_files = create_list(list_name);

	sprintf(conffile_name, "%s/var/lib/dpkg/info/%s.%s", getenv("KBOX"), 
	...
	exclude_files[1] = xasprintf("%s/var/lib/dpkg/info/%s.%s", getenv("KBOX"), 
      package_name, "postrm");
...
static void purge_package(const unsigned package_num)
{
	...
	exclude_files[0] = xasprintf("%s/var/lib/dpkg/info/%s.%s", getenv("KBOX"), 
      package_name, "postrm");
	remove_file_array(remove_files, exclude_files);
	free_array(exclude_files);
...
static void append_control_file_to_llist(const char *package_name, const char *control_name, llist_t **ll)
{
	...
	filename = xasprintf("%s/var/lib/dpkg/info/%s.%s", getenv("KBOX"), 
      package_name, control_name);
...
static void unpack_package(deb_file_t *deb_file)
{
	...
	info_prefix = xasprintf("%s/var/lib/dpkg/info/%s.%s", getenv("KBOX"), 
      package_name, "");
	...
	archive_handle->dpkg__sub_archive->action_data = data_extract_all_prefix;
    // For some reason, dpkg_buffer provides a prefix for file extraction
    snprintf (prefix, MAX_FILENAME - 1, "%s/", getenv ("KBOX"));
	archive_handle->dpkg__sub_archive->dpkg__buffer = (char*)prefix; 
	archive_handle->dpkg__sub_archive->ah_flags |= ARCHIVE_UNLINK_OLD;
	unpack_ar_archive(archive_handle);

	/* Create the list file */
	list_filename = xasprintf("%s/var/lib/dpkg/info/%s.%s", getenv ("KBOX"), 
      package_name, "list");
	out_stream = xfopen_for_write(list_filename);
	while (archive_handle->dpkg__sub_archive->passed) {
		/* the leading . has been stripped by data_extract_all_prefix already */
printf ("X %s\n", archive_handle->dpkg__sub_archive->passed->data);
...
/*	puts("(Reading database ... xxxxx files and directories installed.)"); */
    snprintf (status_file_name, MAX_FILENAME - 1, 
      "%s/var/lib/dpkg/status", getenv("KBOX"));
	index_status_file(status_file_name);
